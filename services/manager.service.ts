        ...
        adminLogin: {
            ...
            async handler(ctx: Context<{ email: string; password: string }>) {
                try {
                    ...
                    if (!user) {
                        throw new MoleculerError("Admin not found", 404, "NOT_FOUND");
                    }

                    const validPassword = await this.verifyPassword(password, user.password);
                    if (!validPassword) {
                        this.logger.warn("Admin password mismatch", { email });
                        throw new MoleculerError("Invalid admin credentials", 401, "UNAUTHORIZED");
                    }
        ...
        createAdmin: {
            ...
            async handler(ctx: Context<{ email: string; password: string }>) {
                try {
                    ...
                    if (existing) {
                        throw new MoleculerError("Admin already exists", 409, "CONFLICT");
                    }
    ...
    async started() {
        ...
        try {
            ...
            if (!existingSuperAdmin) {
                this.logger.info("No superadmin found. Seeding superadmin account...");
                const hashedPassword = await this.hashPassword(SUPER_ADMIN_PASSWORD);
