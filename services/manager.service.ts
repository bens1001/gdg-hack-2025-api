...
const managerService: ServiceSchema<ServiceSettings, ServiceThis> = {
        ...
        login: {
            ...
            async handler(ctx: Context<{ email: string; password: string }>) {
                try {
                    ...
                    if (!ADMIN_PASSWORD) {
                        throw new MoleculerError("Admin password not set", 500, "SERVER_ERROR");
                    }
                    ...
                    if (!manager) {
                        throw new MoleculerError("Manager not found", 404, "NOT_FOUND");
                    }

                    const validPassword = await this.verifyPassword(password, manager.password);
                    if (!validPassword) {
        ...
        create: {
            ...
            async handler(ctx: Context<{ email: string; password: string }>) {
                try {
                    ...
                    if (existing) {
                        throw new MoleculerError("Manager already exists", 409, "CONFLICT");
                    }
    ...
    methods: {
        generateToken(user: any) {
            return jwt.sign({ id: user._id, email: user.email, role: user.roles }, JWT_SECRET, {
                expiresIn: "1h",
            });
        },
    ...
    async started() {
        ...
        try {
            if (!ADMIN_EMAIL || !ADMIN_PASSWORD) {
                this.logger.warn("ADMIN_EMAIL or ADMIN_PASSWORD not set. Skipping seeding.");
                return;
            }
