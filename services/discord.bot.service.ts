...
const discordBotService: ServiceSchema<DiscordBotSettings, DiscordBotThis> = {
    ...
    methods: {
        ...
        handleInteractions(this: DiscordBotThis) {
            this.client?.on("interactionCreate", async (interaction) => {
                ...
                try {
                    ...
                    const discordMember = await this.broker.call("member.getMemberByDiscordId", {
                    ...
                    await command.execute(this.client, interaction, this.broker);
                } catch (error) {
        ...
        async loadCommands(this: DiscordBotThis) {
            ...
            const rest = new REST({ version: "9" }).setToken(this.settings.discordToken);
            try {
                await rest.put(
                    Routes.applicationGuildCommands(this.settings.clientId, this.settings.guildId),
                    { body: commands },
                );
