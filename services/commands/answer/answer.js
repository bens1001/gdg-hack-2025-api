	...
	data: new SlashCommandBuilder()
		...
		.addIntegerOption(option =>
			option.setName('skill_level')
				.setDescription('Skill level (e.g., 1 for beginner, 2 for intermediate, 3 for advanced).')
				.setRequired(true)
		),
	async execute(client, interaction, args) {
		...
		try {
			...
			const embed = new discord.MessageEmbed()
				...
				.setDescription(`Voice channel **${voiceChannel.name}** created for discussion on question ID: **${questionId}** at skill level **${skillLevel}**.`)
				.setColor("GREEN");
